    ...
    let env =
      match sum with
        Env_empty ->
          Env.empty
      | Env_value(s, id, desc) ->
          Env.add_value ~is_spec:true id (Subst.value_description subst desc)
                        (env_from_summary s subst)
      | Env_value_spec(s, id, desc) ->
          Env.add_value ~is_spec:false id (Subst.value_description subst desc)
                        (env_from_summary s subst)
      | Env_type(s, id, desc) ->
